...
3. Pick built-in instruction template (`Student`, `Brief`, `Formal`, `Expert`) or save your own custom template.
4. Add or tweak instruction text if needed.
5. Keep **Preview before fill** enabled and click **Analyze (Preview)**.
6. Review suggested answers, uncheck anything you do not want, then click **Apply Selected**.
7. Open the **Debug** panel in popup to inspect raw model output and local mapping details.
8. Or disable preview and use one-click **Analyze and Fill** / shortcut **Ctrl+Shift+Y** (macOS: **Command+Shift+Y**).
